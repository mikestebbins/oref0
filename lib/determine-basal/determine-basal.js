...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    if ( microBolusAllowed && currenttemp && iob_data.lastTemp && currenttemp.rate != iob_data.lastTemp.rate && lastTempAge > 10 && currenttemp.duration ) {
        ...
        return tempBasalFunctions.setTempBasal(0.20, 0, profile, rT, currenttemp);
    }
    ...
    // predictive low glucose suspend mode: BG is / is projected to be < threshold
        ...
        return tempBasalFunctions.setTempBasal(0.20, durationReq, profile, rT, currenttemp);
    }
    ...
    if (eventualBG < min_bg) { // if eventual BG is below target:
        ...
        if ( minDelta > expectedDelta && minDelta > 0 && !carbsReq ) {
            ...
            if (naive_eventualBG < 40) {
                ...
                return tempBasalFunctions.setTempBasal(0.20, 30, profile, rT, currenttemp);
            }
    ...
    if (iob_data.iob > max_iob) {
        ...
        // Mike added below
        if (rate < 0.20) {
            rT.reason += "adj. req. rate: "+rate+" to MinBasal: "+0.25+", ";
            rate = round_basal(0.20, profile);
        }
